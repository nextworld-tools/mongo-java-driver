...
public class FindPublisherVerification extends PublisherVerification<OldDocument> {
    ...
    public Publisher<OldDocument> createPublisher(final long elements) {
        ...
        MongoCollection<OldDocument> collection = MongoFixture.getDefaultDatabase().getCollection("FindTest");
        run(collection.drop());
        if (elements > 0) {
